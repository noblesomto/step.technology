    ...
    <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div class="lg:col-span-8 lg:order-2">
            <div class="text-center mb-8">
                <h1 class="font-step-heading font-bold text-2xl sm:text-3xl text-step-primary">Register as Corporate Organization</h1>
            ...
            <form action="/register-corporate-organization" method="POST" class="bg-white rounded-lg border border-gray-200 p-6 sm:p-8 space-y-4">
                @csrf
                <div>
                    @if ($errors->has('company_name')) <span class="block text-red-600 text-sm mb-1">{{ $errors->first('company_name') }}</span> @endif
                    <input type="text" name="company_name" placeholder="Organization Name *" required value="{{ old('company_name') }}" class="w-full h-11 rounded-md border-gray-300 focus:border-step-primary focus:ring-step-primary text-sm px-3">
                </div>
                <div>
        ...
        <div class="lg:col-span-4 lg:order-1">
            @include('frontend.layouts.tailwind.register-sidebar', ['active' => 'corporate-organization'])
        </div>
    </div>
</section>
