...
@include('frontend.layouts.tailwind.page-header', ['pageTitle' => $event->event_title, 'breadcrumb' => 'Events'])

<section class="py-16">
    <div class="max-w-3xl mx-auto px-4">
        <article>
            <div class="relative rounded-lg overflow-hidden mb-8">
                <img src="{{ asset('uploads/event/'.$event->event_picture) }}" alt="{{ $event->event_title }}" class="w-full h-80 object-cover">
                <span class="absolute top-4 left-4 bg-step-primary text-white text-xs font-step-heading font-semibold px-3 py-1 rounded-full">{{ $event->created_at->format('j F') }}</span>
            </div>

            <h1 class="font-step-heading font-bold text-2xl sm:text-3xl text-step-primary mb-2">{{ $event->event_title }}</h1>
        ...
        <div class="mt-10 pt-6 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex gap-3">
                ...
                <a href="https://twitter.com/share?url={{ url()->current() }}&text={{ $event->event_title }}" target="_blank" class="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center hover:bg-step-primary hover:text-white transition-colors"><i class="fa fa-twitter"></i></a>
