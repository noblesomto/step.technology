            ...
            <div class="container mx-auto p-1">
		        ...
		        <div class="">
		        	...
		        	<div class="max-w-2xl mx-auto bg-white p-3 md:p-10 mt-4 md:mt-10 mb-20 rounded-lg">
				        <form method="POST" action="/user/account-details" enctype="multipart/form-data">
				            @csrf
				            @method('PUT')
				        ...
				        <div class="mb-4 mt-4">
				            <label class="text-sm font-semibold">Sort Code *</label>
				            @if ($errors->has('sort_code'))
				                <span class="text-red-700 py-1">{{ $errors->first('sort_code') }}</span>
				            @endif
				            <input type="text" id="name" name="sort_code" placeholder="Sort Code" value="{{ $user->sort_code }}" class="w-full px-3 py-2 border border-gray-300 rounded shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
				        </div>
				         ...
				         <div class="mb-6 mt-4">
				           ...
				           <label for="alipay_qr" class="block text-gray-700 text-sm font-bold mb-2">Alipay QR Code</label>
				           @if ($errors->has('alipay_qr'))
				                <span class="text-red-900 my-1">{{ $errors->first('alipay_qr') }}</span>
				            @endif
				            <div> <img class="w-48" src="{{  asset('uploads/profile/'.$user->alipay_qr) }}"> </div>
						    <input type="file" name="alipay_qr" id="alipay_qr"
