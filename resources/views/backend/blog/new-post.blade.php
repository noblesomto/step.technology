...
<div class="mb-6">
    <h1 class="font-step-heading font-bold text-2xl text-gray-900">New Blog Post</h1>
</div>
...
<div class="bg-white rounded-lg border border-gray-200 p-6 sm:p-8">
    <form action="/blogpost/new-post" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-4">
            ...
            @endif
            <input type="text" name="blog_title" placeholder="Blog Post Title" value="{{ old('blog_title') }}" required class="w-full max-w-xl h-11 rounded-md border-gray-300 focus:border-step-primary focus:ring-step-primary text-sm px-3">
        </div>
        ...
        <div class="mb-4">
            ...
            @endif
            <input type="file" name="blog_picture" required class="text-sm text-gray-600 file:mr-4 file:py-2.5 file:px-4 file:rounded-md file:border-0 file:bg-step-primary/10 file:text-step-primary file:font-semibold hover:file:bg-step-primary/20">
        </div>

        @include('backend.layouts.tailwind.trix-editor', ['name' => 'blog_body', 'label' => 'Blog Body'])
