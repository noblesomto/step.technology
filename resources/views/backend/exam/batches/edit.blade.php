...
<div class="mb-6">
    <h1 class="font-step-heading font-bold text-2xl text-gray-900">Edit Exam Sitting</h1>
...
<div class="bg-white rounded-lg border border-gray-200 p-6 sm:p-8 max-w-xl">
    <form action="/admin/exam-batches/{{ $batch->id }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        @include('backend.exam.batches._form', ['batch' => $batch])
